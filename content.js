// Instagram Message Assistant - Content Script
console.log('📱 Instagram Message Assistant loaded!');

// Your working message scraping function (adapted for Instagram)
function scrapeMessagesWithContext() {
// Instagram Message Scraper - Minimal version matching your Messenger approach
    // Find message container (equivalent to your Messenger container)
    const container = document.querySelector('[aria-label*="Messages in conversation"]');
    if (!container) return [];
    
    // Extract partner name from container
    const partnerName = container.getAttribute('aria-label')
        .match(/Messages in conversation with (.+)/)?.[1] || 'Contact';
    
    // Get all message rows (equivalent to your data-scope="messages_table")
    const messageNodes = container.querySelectorAll('[role="row"]');
    const messages = [];
    let lastSender = 'Unknown Sender';

    messageNodes.forEach(node => {
        // Extract sender (equivalent to your h5 logic)
        const nodeText = node.textContent;
        if (nodeText.includes('You sent')) {
            lastSender = 'You';
        } else if (nodeText.includes(partnerName) || nodeText.includes('replied to you')) {
            lastSender = partnerName;
        }

        // Extract message content (equivalent to your .x1gslohp)
        const contentDivs = node.querySelectorAll('div[dir="auto"]');
        for (const div of contentDivs) {
            const text = div.textContent?.trim();
            
            // Filter out UI text and get actual messages
            if (text && 
                text.length > 0 && 
                !text.includes('Enter') && 
                ...
                !text.includes('Details and actions')) {
                
                // Normalize sender (same logic as your Messenger code)
                let normalizedSender = lastSender;
                if (lastSender === 'You' || lastSender.startsWith('You sent')) {
                    normalizedSender = 'Me';
...
console.log('🤖 Instagram Chat AI Assistant loaded!');
...
async function callDeepSeekAPI(conversation, guide, context = '', stage = 'Opening') {
    ...
    try {
        ...
        const systemPrompt = `
You are an expert sales coach specializing in closing web design and digital marketing deals via Instagram DM.
...
function createSidePanel() {
    ...
    panel.innerHTML = `
        <div id="closing-coach-header" style="
            background: linear-gradient(135deg, #E91E63 0%, #FF4081 50%, #F50057 100%);
            color: white;
            padding: 16px;
            display: flex;
        ...
        <div style="padding: 16px; overflow-y: auto; max-height: 540px;">
            <div style="margin-bottom: 10px;">
                <label for="stage-select" style="font-size:13px;color:#E91E63;font-weight:600;">Sales Stage:</label>
            ...
            <div style="margin-bottom: 10px;">
                <label for="context-input" style="font-size:13px;color:#E91E63;font-weight:600;">Context (optional):</label>
...
function analyzeConversation(context = '', stage = 'Opening') {
      ...
      <div id="dynamic-loading-message" style="display: flex; flex-direction: column; align-items: center; margin-bottom: 16px;">
        <div style="width: 100%; display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
          <div style="flex: 1; background: #f0f0f0; border-radius: 10px; height: 8px; overflow: hidden;">
            <div id="progress-bar" style="width: 0%; height: 100%; background: linear-gradient(90deg, #E91E63, #FF4081); border-radius: 10px; transition: width 0.3s ease-out; position: relative; overflow: hidden; box-shadow: 0 0 10px rgba(233, 30, 99, 0.3);">
          ...
          <div id="progress-text" style="font-size: 14px; color: #E91E63; font-weight: 600; min-width: 40px; text-align: right; animation: bounceText 0.5s ease-out;">0%</div>
        </div>
        <div id="loading-msg-text" style="font-size: 13px; color: #E91E63; font-weight: 500; letter-spacing: 0.2px; text-align: center; max-width: 280px; line-height: 1.4;">${loadingMessages[0]}</div>
      </div>
    ...
    console.log(`[INSTAGRAM] Messages:`, formatConversationAsTranscript(messages));
                ...
                if (aiResponse && Array.isArray(aiResponse) && aiResponse.length > 0) {
                    ...
                    suggestionsContainer.innerHTML = `
                        <div style=\"text-align:center;color:#dc3545;font-size:13px;padding:20px;\">
                            ${errorMsg}<br>
                            <button id=\"try-again-btn\" style=\"margin-top:12px;background:linear-gradient(135deg, #E91E63 0%, #FF4081 100%);color:white;border:none;border-radius:6px;padding:8px 16px;font-size:13px;cursor:pointer;box-shadow:0 2px 8px rgba(233, 30, 99, 0.3);\">🔄 Try Again</button>
                        </div>
                    `;
    ...
    const suggestionsHtml = suggestions.map((s, index) => {
        ...
        if (typeof s.reason === 'object' && s.reason !== null) {
            ...
            reasonHtml = `<div style="border-top:1px solid #e1e5e9;margin-top:12px;padding-top:10px;background:#f6f8fa;border-radius:0 0 8px 8px;">
                <div style="font-size:12px;font-weight:600;color:#E91E63;margin-bottom:4px;display:flex;align-items:center;gap:6px;">
                    🧠 Why this works
                    <span style="font-size:11px;font-weight:400;color:#888;">(for your learning, not for copying)</span>
                </div>
        ...
        return `
        <div style="position:relative;background:#f8f9fa;border:1px solid #e9ecef;border-radius:8px;padding:18px 16px 10px 16px;margin-bottom:18px;box-shadow:0 2px 8px rgba(233,30,99,0.08);">
            <button
                class="copy-btn"
            ...
            <div style="font-size:13px;color:#E91E63;font-weight:600;margin-bottom:6px;">
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        case 'showPanel':
            // Toggle panel - if exists, remove it; if not, create it
            const existingPanel = document.getElementById('closing-coach-panel');
            if (existingPanel) {
        ...
        case 'refresh':
            ...
            const refreshPanel = document.getElementById('closing-coach-panel');
            if (refreshPanel) {
                refreshPanel.remove();
            }
            setTimeout(() => {
...
function initializeExtension() {
    if (window.location.hostname === 'www.instagram.com') {
        console.log('✅ Closing Coach loaded on Instagram');
        console.log('💡 Click the extension icon to show/hide the AI panel');
        console.log('🔧 Extension version: 0.1.0');
        console.log('📱 Current URL:', window.location.href);

        // Don't auto-create panel anymore - let user control it via popup
        // createSidePanel();
    } else {
        console.log('❌ Closing Coach not loaded - not on Instagram');
